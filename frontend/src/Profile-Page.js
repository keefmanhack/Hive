import React from "react";
import ImageGallary from './ImageGallary';

class ProductPage extends React.Component{
	render(){
		const genPer = 'data:image/jpeg;base64,';
		const genPerArr = [genPer, genPer, genPer];
		return(
			<div className='profile-page-container'>
				<div className='row'>
					<div className='col-lg-4'>
						<div className='hyve-section'>
							<div style={{position: 'relative'}} className='name-location'>
								<img src={genPer} />
								<h1>Sarah Steel</h1>
								<h2>New York, NY United States</h2>
							</div>

							<div className='row skills'>
								<div className='col-lg-4'>
									<button className='selected' >Musician</button>
								</div>
								<div className='col-lg-4'>
									<button >Artist</button>
								</div>
								<div className='col-lg-4'>
									<button >Dog Walker</button>
								</div>
							</div>

							<div className='skill-section'>
								<h1>Musician</h1>
								<h2>Available</h2>
								<ul style={{listStyle: 'none'}}>
									<li>
										Tues. June 22st
									</li>
									<li>
										Wed. June 23st
									</li>
									<li>
										Thur. June 24st
									</li>
								</ul>
								<h5>Hyver for <span style={{fontWeight: 700}}>7 Months</span></h5>
								<h5>Averages <span style={{fontWeight: 700}}>4.5 Stars</span></h5>
								<h5>Total of <span style={{fontWeight: 700}}>105 Reviews</span></h5>
							</div>
						</div>
					</div>
					<div className='col-lg-8'>
						<div className='text-section'>
							<p>
								mply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown panged. It wlkjsdlkjfalksdjfalksdjflkajsd lkasdjflk laksdjf asdkjf laksdjf lakm passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.
							</p>
							<div className='row' style={{padding: '0 10px'}}>
								<div className='col-lg-5'>
									<div className='experience' style={{position: 'relative'}}>
										<img src={genPer}/>
										<h1>Gannon University</h1>
										<h2>Student</h2>
										<h3>From 2017 to 2021</h3>
									</div>
								</div>
								<div className='col-lg-7'>
									<ImageGallary
									 images={genPerArr}
									 largeImageHeight={250}
									 editable={false}
									 gallarySpacing={'col-lg-4'}
									/>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		);
	}
}

export default ProductPage;